...
}`;async function Ee(){const t=Q(),n={"X-WP-Nonce":Y()};try{const[a,c,m,r]=await Promise.all([fetch(`${t}/simulator/sessions`,{headers:n,credentials:"same-origin"}),fetch(`${t}/video-training/sessions`,{headers:n,credentials:"same-origin"}),fetch(`${t}/roleplays/sessions`,{headers:n,credentials:"same-origin"}),fetch(`${t}/game/sessions`,{headers:n,credentials:"same-origin"})]),[l,h,d,j]=await Promise.all([a.ok?a.json():{data:{sessions:[]}},c.ok?c.json():{data:{sessions:[]}},m.ok?m.json():{data:[]},r.ok?r.json():{data:[]}]),x=(o,p=null)=>{if(Array.isArray(o))return o;if(o?.data){if(p&&o.data[p])return Array.isArray(o.data[p])?o.data[p]:[];if(Array.isArray(o.data))return o.data;if(o.data.sessions&&Array.isArray(o.data.sessions))return o.data.sessions}return[]};return{simulator:x(l,"sessions"),video:x(h,"sessions"),roleplay:x(d),games:x(j)}}catch(a){return console.error("[CoachingIntelligence] Failed to fetch sessions:",a),{simulator:[],video:[],roleplay:[],games:[]}}}async function Te(){const t=Q(),n={"X-WP-Nonce":Y()};try{const[a,c,m,r]=await Promise.all([fetch(`${t}/simulator/scenarios`,{headers:n,credentials:"same-origin"}),fetch(`${t}/video-training/scenarios`,{headers:n,credentials:"same-origin"}),fetch(`${t}/roleplays`,{headers:n,credentials:"same-origin"}),fetch(`${t}/smartbriefing/templates`,{headers:n,credentials:"same-origin"})]),[l,h,d,j]=await Promise.all([a.ok?a.json():{data:{scenarios:[]}},c.ok?c.json():{data:{scenarios:[]}},m.ok?m.json():{data:[]},r.ok?r.json():{data:[]}]),x=(o,p=null)=>{if(Array.isArray(o))return o;if(o?.data){if(p&&o.data[p])return Array.isArray(o.data[p])?o.data[p]:[];if(Array.isArray(o.data))return o.data}return[]};return{simulator:x(l,"scenarios"),video:x(h,"scenarios"),roleplay:x(d),briefingTemplates:x(j)}}catch(a){return console.error("[CoachingIntelligence] Failed to fetch scenarios:",a),{simulator:[],video:[],roleplay:[],briefingTemplates:[]}}}function Ie(t){const n=Array.isArray(t?.simulator)?t.simulator:[],a=Array.isArray(t?.video)?t.video:[],c=Array.isArray(t?.roleplay)?t.roleplay:[],m=Array.isArray(t?.games)?t.games:[],r=[...n.map(s=>({...s,module:"Szenario-Training"})),...a.map(s=>({...s,module:"Wirkungs-Analyse"})),...c.map(s=>({...s,module:"Live-Simulation"})),...m.map(s=>({...s,module:"Rhetorik-Gym"}))],l=r.length,h={"Szenario-Training":n.length,"Wirkungs-Analyse":a.length,"Live-Simulation":c.length,"Rhetorik-Gym":m.length},d={overall:[],communication:[],content:[],structure:[],confidence:[],fillerWords:[]};n.forEach(s=>{if(s.overall_score!=null){const i=parseFloat(s.overall_score);d.overall.push(i<=10?i*10:i)}try{const i=s.summary_feedback||s.summary_feedback_json,u=typeof i=="string"?JSON.parse(i):i;u?.scores&&(u.scores.content!=null&&d.content.push(u.scores.content*10),u.scores.structure!=null&&d.structure.push(u.scores.structure*10))}catch{}}),a.forEach(s=>{if(s.overall_score!=null){const i=parseFloat(s.overall_score);d.overall.push(i<=10?i*10:i)}}),c.forEach(s=>{try{const i=typeof s.feedback_json=="string"?JSON.parse(s.feedback_json):s.feedback_json,u=i?.rating?.overall??i?.rating?.gesamteindruck??i?.overall_score??i?.overallScore??i?.score;if(u!=null){const te=parseFloat(u)<=10?parseFloat(u)*10:parseFloat(u);d.overall.push(te)}const v=i?.rating?.communication??i?.rating?.kommunikation;v!=null&&d.communication.push(parseFloat(v)*10)}catch{}});let j=0,x=0;m.forEach(s=>{s.score!=null&&d.overall.push(parseFloat(s.score)),s.filler_count!=null&&(j+=parseInt(s.filler_count),x++)});const o=s=>s.length>0?s.reduce((i,u)=>i+u,0)/s.length:null,p={Gesamt:o(d.overall),Kommunikation:o(d.communication),Inhalt:o(d.content),Struktur:o(d.structure)},D=new Date;D.setDate(D.getDate()-30);const z=[],_=[],$=s=>{const i=s.created_at||s.started_at||s.updated_at;if(!i)return null;const u=new Date(i);return isNaN(u.getTime())?null:u},A=s=>{if(s.overall_score!=null){const i=parseFloat(s.overall_score);return i<=10?i*10:i}if(s.score!=null)return parseFloat(s.score);try{const i=typeof s.feedback_json=="string"?JSON.parse(s.feedback_json):s.feedback_json,u=i?.rating?.overall??i?.rating?.gesamteindruck??i?.overall_score;if(u!=null){const v=parseFloat(u);return v<=10?v*10:v}}catch{}return null};r.forEach(s=>{const i=$(s),u=A(s);i&&u!=null&&(i>=D?z.push(u):_.push(u))});const L=o(z),E=o(_),P=L!=null&&E!=null&&E>0?(L-E)/E*100:null,C={},w={};[...n,...a,...c].forEach(s=>{try{let i=s.summary_feedback||s.summary_feedback_json||s.feedback_json||s.analysis_json;typeof i=="string"&&(i=JSON.parse(i)),i?.strengths&&i.strengths.forEach(u=>{const v=u.toLowerCase().substring(0,50);C[v]=(C[v]||0)+1}),(i?.improvements||i?.weaknesses)&&(i.improvements||i.weaknesses||[]).forEach(u=>{const v=u.toLowerCase().substring(0,50);w[v]=(w[v]||0)+1})}catch{}});const f=Object.entries(C).sort((s,i)=>i[1]-s[1]).slice(0,5).map(([s])=>s),G=Object.entries(w).sort((s,i)=>i[1]-s[1]).slice(0,5).map(([s])=>s),I=r.map(s=>({session:s,date:$(s)})).filter(({date:s})=>s!==null).sort((s,i)=>i.date-s.date),R=I[0]?.date?I[0].date.toLocaleDateString("de-DE"):null,y=I[0]?.date?Math.floor((Date.now()-I[0].date)/(1e3*60*60*24)):null,b=[];return[...n,...m].forEach(s=>{try{let i=s.analysis||s.audio_analysis||s.analysis_json;typeof i=="string"&&(i=JSON.parse(i)),i?.pacing?.rating&&i.pacing.rating!=="optimal"&&(b.includes(i.pacing.rating)||b.push(i.pacing.rating))}catch{}}),{totalSessions:l,moduleBreakdown:h,averageScores:p,recentTrend:P,topStrengths:f,topWeaknesses:G,fillerWordAverage:x>0?j/x:null,pacingIssues:b,lastSessionDate:R,daysSinceLastSession:y??999,rawSessions:t}}function Fe(t){const n=Array.isArray(t?.simulator)?t.simulator:[],a=Array.isArray(t?.video)?t.video:[],c=Array.isArray(t?.roleplay)?t.roleplay:[],m=Array.isArray(t?.briefingTemplates)?t.briefingTemplates:[];let r=`
## VERFÜGBARE TRAININGS-SZENARIEN

`;return n.length>0&&(r+=`### Szenario-Training (strukturiertes Q&A mit Feedback)
`,n.forEach(l=>{const h=l.difficulty||l.meta?.difficulty||"Mittel";r+=`- ID:${l.id} "${l.title}" [${h}]${l.description?` - ${l.description.substring(0,100)}`:""}
`}),r+=`
...
`,r}async function De(t,n){const{totalSessions:a}=t;if(a<3){console.log("[CoachingIntelligence] New user - using welcome prompt");const l=Ce();try{return{...await V(l),isWelcome:!0,generatedAt:new Date().toISOString()}}catch(h){return console.error("[CoachingIntelligence] Failed to generate welcome coaching:",h),_e()}}const c=Fe(n),r=Ae(t)+`
`+c+`
