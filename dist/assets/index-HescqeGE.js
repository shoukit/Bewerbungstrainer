...
### Module-Aktivität
${Object.entries(a).map(([j,z])=>`- ${j}: ${z} Sessions`).join(`
`)}

### Durchschnittliche Bewertungen (0-100)
${Object.entries(m).map(([j,z])=>`- ${j}: ${z!==null?Math.round(z):"Keine Daten"}`).join(`
`)}
...
`,l}async function _e(t,s,r=null){const{totalSessions:a}=t;if(a<3){console.log("[CoachingIntelligence] New user - using welcome prompt");const u=Ee();try{return{...await V(u),isWelcome:!0,generatedAt:new Date().toISOString()}}catch(c){return console.error("[CoachingIntelligence] Failed to generate welcome coaching:",c),$e()}}const m=Ie(s),o=Ce(t,r)+`
`+m+`
